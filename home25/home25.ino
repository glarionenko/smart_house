...
void simply_trigger_it(int tr_num, int pin_num) {
  ...
  if (trigger[tr_num]) {
    ...
    if ((tr_num == tr_light_bathroom_floor1) && (readHere == 0)) {
      digitalWrite(fan_bathroom_floor1, 1);
    }
    if ((tr_num == tr_light_bathroom_floor2) && (readHere == 0)) {

      digitalWrite(fan_bathroom_floor2, 1);

    }
    //
    if ((tr_num == tr_light_bathroom_floor1) && (readHere == 1)) {
      started_1 = 1;
      timer_1 = millis();
    }
    if ((tr_num == tr_light_bathroom_floor2) && (readHere == 1)) {
      started_2 = 1;
      timer_2 = millis();
    }
    //
